...
pub struct SleepTimer {
    ...
    pub last_position_ms: Option<u32>,
...
impl SleepTimer {
    ...
    pub fn status_text(&self, locale: crate::i18n::Locale) -> String {
        use crate::i18n::gettext;

        if self.waiting_for_song_end {
            return gettext(locale, "Sleep timer: waiting for song to finish").to_string();
        }
        match self.setting {
            SleepTimerSetting::EndOfTrack => {
                gettext(locale, "Sleep timer: end of track").to_string()
            }
            SleepTimerSetting::Duration(_) => {
                if let Some(rem) = self.remaining_duration() {
                    ...
                    let suffix = if self.wait_for_song_end {
                        format!(" ({})", gettext(locale, "waits for song end"))
                    } else {
                        String::new()
                    };
                    let prefix = gettext(locale, "Sleep timer");
                    if mins > 0 {
                        format!("{prefix}: {mins}m {secs:02}s{suffix}")
                    } else {
                        format!("{prefix}: {secs}s{suffix}")
                    }
                } else {
                    gettext(locale, "Sleep timer").to_string()
                }
            }
        }
    }
        ...
        .show(ctx, |ui| {
            super::widgets::menu_frame(&palette).show(ui, |ui| {
                ...
                ui.horizontal(|ui| {
                    ...
                    theme::text(
                        ui,
                        crate::i18n::gettext(app.locale, "Sleep timer"),
                        theme::bold(16.0),
                        palette.text,
                    );
                    if let Some(ref timer) = app.sleep_timer {
                        ui.with_layout(Layout::right_to_left(Align::Center), |ui| {
                            let text = match &timer.setting {
                                SleepTimerSetting::EndOfTrack => {
                                    crate::i18n::gettext(app.locale, "End of track").to_string()
                                }
                                SleepTimerSetting::Duration(_) => {
                ...
                response.widget_info(|| {
                    egui::WidgetInfo::selected(
                        ...
                        ui.is_enabled(),
                        wait_active,
                        crate::i18n::gettext(app.locale, "Wait for song to finish"),
                    )
                });
                ...
                painter.text(
                    ...
                    egui::Align2::LEFT_CENTER,
                    crate::i18n::gettext(app.locale, "Let the current song finish before stopping"),
                    theme::regular(11.0),
                    palette.dim,
                );
                if response.clicked() {
                ...
                if app.sleep_timer.is_some() {
                    ...
                    response.widget_info(|| {
                        egui::WidgetInfo::labeled(
                            ...
                            ui.is_enabled(),
                            crate::i18n::gettext(app.locale, "Turn off timer"),
                        )
                    });
                    theme::focus_ring(ui, &response);
                    let icon_rect = Rect::from_center_size(
